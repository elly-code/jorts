...
public class Jorts.TextView : Granite.HyperTextView {
    ...
    private ListPrefix _cached_list_prefix = ListPrefix.DISABLED;
    public ListPrefix list_item_prefix {
        get { return _cached_list_prefix; }
        set {
            if (_cached_list_prefix == value) {
                return;
            }

            migrate_list_prefixes ( value);
            _cached_list_prefix = value;
    ...
    private void ensure_tags () {
        ...
        var list_item_tag = buffer.tag_table.lookup (LIST_TAG_NAME);

        if (list_item_tag == null) {
            buffer.create_tag (LIST_TAG_NAME,
                "indent", -indent_width,
    ...
    public void restore_list_item_indentation () {
        ...
        buffer.remove_tag_by_name (LIST_TAG_NAME, start, end);
        ...
        for (int line_number = 0; line_number < line_count; line_number++) {
            ...
            buffer.apply_tag_by_name (LIST_TAG_NAME, line_start, line_end);
        }
    }
    ...
    public void toggle_list () {
        ...
        var selected_is_list = this.is_list (first_line, last_line, list_item_prefix);
    ...
    private void migrate_list_prefixes (ListPrefix new_prefix) {
        if (new_prefix == ListPrefix.DISABLED) {
            Gtk.TextIter start, end;
            buffer.get_bounds (out start, out end);
            buffer.remove_tag_by_name (LIST_TAG_NAME, start, end);
     ...
     */
    private bool is_list (int first_line, int last_line, string list_item_prefix) {
    ...
    private void set_list (int first_line, int last_line) {
        ...
        for (int line_number = first_line; line_number <= last_line; line_number++) {
            ...
            if (!this.has_prefix (line_number)) {
                buffer.get_iter_at_line_offset (out line_start, line_number, 0);
                buffer.insert (ref line_start, list_item_prefix, -1);
            }
    ...
    private void remove_prefix (int line_number) {
        ...
        line_end = line_start.copy ();
        line_end.forward_to_line_end ();
        buffer.remove_tag_by_name (LIST_TAG_NAME, line_start, line_end);
    }
